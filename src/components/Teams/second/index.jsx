...
const Second = ({ second }) => {
  ...
  const visiblePhotos = second.slice(currentIndex, currentIndex + 4);
  const totalPages = Math.ceil(second.length / 4); // Calculate the total number of pages
  ...
  return (
    <div className="container">
      <div className="text-container">
        ...
        <button disabled="disabled">Year-2nd</button>
      </div>
      <div className="photo-container">
        ...
        <div className="radio-buttons">
          {[...Array(Math.ceil(second.length / 4))].map((_, index) => (
            <label key={index}>
              <input
                type="radio"
                name="team-radio"
